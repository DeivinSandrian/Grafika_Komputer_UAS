...
function applyOversizeScale(model, modelPath) {
    for (const key in model_scale) {
        if (modelPath.includes(key)) {
            model.scale.setScalar(model_scale[key]);
            return;
        }
    }
}

function createGallery(offsetX = 0, modelList = []) { // Awal bukan AI, setelahnya AI
